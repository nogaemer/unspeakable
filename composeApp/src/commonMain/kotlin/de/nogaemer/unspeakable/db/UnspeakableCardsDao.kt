...
interface UnspeakableCardsDao {
    ...
    suspend fun getAllCards(lang: String): List<UnspeakableCard>

    @Query("SELECT * FROM cards WHERE language = :lang AND id = :id")
    suspend fun getCardById(lang: String, id: Int): UnspeakableCard?

    @Query("SELECT * FROM cards WHERE language = :lang AND word = :word")
    suspend fun getCardByWord(lang: String, word: String): UnspeakableCard?

    @Query("SELECT * FROM cards WHERE language = :lang AND category = :category")
    suspend fun getCardsByCategory(lang: String, category: String): List<UnspeakableCard>

    @Query("SELECT * FROM cards WHERE language = :lang AND category = :category AND word = :word")
    suspend fun getCardsByCategoryAndWord(lang: String, category: String, word: String): UnspeakableCard?

    @Query("SELECT * FROM cards WHERE language = :lang ORDER BY RANDOM() LIMIT :limit")
    suspend fun getRandomCards(lang: String, limit: Int): List<UnspeakableCard>

    @Query("SELECT * FROM cards WHERE language = :lang ORDER BY RANDOM() LIMIT 1")
    suspend fun getRandomCard(lang: String): UnspeakableCard?
